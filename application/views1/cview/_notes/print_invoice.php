...
<?php 
$get_id = $this->View_model->get_id($this->session->userdata('post_id'));
foreach($get_id as $edeit)
		  {
		  ?>
	<span class="left">Tel: <?php echo $set->phone; ?></span> 
	<span class="right">Reference No. <?=$edeit->h_reference;?></span>
    <span class="left">Seller: <?=$edeit->h_seller;?></span> 
	<span class="right">Date: <?=$edeit->h_date;?></span>   <!-- <div style="clear:both;"></div>-->
...
<?php } ?> 
	...
	<tbody>
    	  <?php $i = 1; ?>
    		<?php foreach ($get_hold_detail as $detail) {
				if($edeit->h_inv_id == $detail->h_invo_id){
				 ?>
        	
			<tr>
            	<td style="text-align:left; padding-bottom:17px !important; width:20px;"><?php echo $i; ?></td>
                <td style="text-align:left; width:150px; line-height:15px;">&nbsp;<?=$detail->h_name_k;?> <br />
                 &nbsp;<?=$detail->h_name;?> </td>
                <td style="text-align:center; padding-bottom:17px !important; width:30px;"><?=$detail->h_qty;?></td>
                <td style="text-align:right; padding-bottom:17px !important; width:70px; ">$ <?=$detail->h_price;?></td>
                <td style="text-align:right; padding-bottom:17px !important; width:70px; ">$ <?=$detail->h_amount;?></td>
			</tr> 
       		<?php $i++; }  ?>
			<?php } ?>
    	</tbody> 
   ...
   <td colspan="2" style="padding-top:0px; border-top:1px solid #000; text-align:right; font-weight:bold;"> $ <?=$edeit->h_total_amount;?></td>
    </tr>
    <tr>    
   	 <td colspan="2" style="text-align:left; font-weight:bold; padding-top:0px;">បញ្ចុះតម្លៃ​ / Discount (%)</td>
   <td colspan="2" style="padding-top:0px; text-align:right; font-weight:bold;"> % <?=$edeit->h_total_discount;?></td>
    </tr>
    <tr>
    ...
    <td colspan="2" style="font-size:14px; text-align:right; font-weight:bold;"> ៛ <?=$edeit->h_grand_total_khr;?></td>
    </tr>

      <tr>    
   	 <td colspan="3" style="font-size:12px; text-align:left; font-weight:bold; padding-top:0px;">Grand Total (USD)</td>
   <td colspan="2" style="font-size:14px; padding-top:0px; text-align:right; font-weight:bold;"> $ <?=$edeit->h_grand_total;?></td>
    </tr>
