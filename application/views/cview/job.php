     ...
     <?php foreach($show_job_detail as $job){ ?>
    	<div class="header-view-job">
            <div class="view-job-left">
                ...
                <div class="a2a_kit a2a_kit_size_32 a2a_default_style" data-a2a-url="http://aseanhr.com/index.php/Pages/job/<?=$this->uri->segment(3)?>" data-a2a-title="Example Page">
  ...
  <div class="modal-content">
    ...
    <form action="<?=site_url('Pages/apply_job');?>" method="post" enctype="multipart/form-data">
        ...
        <div class="my_apply_job">
              ...
              <?php 
			  	foreach($show_job_detail as $job){
			  		foreach($show_company as $com){
						if($com->com_id == $job->comp_id){ 
			 ?>
              <input type="text" name="apply_to" value="<?=$com->com_name;?>"/>
             <?php 
			  			} 
					}
				}
        ...
        <div class="my_apply_job">
          <label>Send to<span style="margin-left:55px;">:</span></label>
          <?php 
			  foreach($show_job_detail as $job){
				  foreach($show_company as $com){
					  if($com->com_id == $job->comp_id){ 
		   ?>
						<input type="text" name="send_to" value="<?=$com->com_email;?>"/>
		   <?php 
					  } 
				  }
			  }
        ...
        <p style="padding:0px 0px 0px 10px; font-size:16px; color:#00C; font-weight:bold;">Please select your CV below!</p>
        <span style="margin-right:65px; margin-left:5px;"><input checked type="radio" name="checked" value="profile"/> From your account</span>
        <input type="radio" name="checked" value="brow"/> From your computer<br/>
        </div>
        <div id="profile">
            <div class="my_apply_job">
                <label>CV Name:</label>
        ...
        <div class="my_apply_job">
          <label style="font-weight:bold; color:#00C;">Description:</label>
          <textarea name="descripiton" role="20" style="width:500px; height:90px;"></textarea>
        </div>
